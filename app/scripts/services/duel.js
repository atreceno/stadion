...
helper.factory('Duel', function() {
	...
	var T = {
		...
		// brackets: duel only (for now.. crossovers could
		// change that)
		// Bracket numbers are locked down for the the following
		// logical equivalence:
		// numBrackets === n <=> lastBracket === n <=> n-tuple
		// elimination
		WB : 1,
		LB : 2,
		TB : 3
	...
	// matches are stored in a sorted array rather than an ID ->
	// Match map
	// This is because ordering is more important than being
	// able to access any match
	// at any time. Looping to find the one is also quick
	// because ms is generally short.
	 ...
	 * common.js
	 */
	// to ensure first matches first and (for most part) forEach
	// scorability
	// score section desc, else round desc, else match number
	// desc
	...
	// internal sorting of zipped player array with map score
	// array : zip(g.p, g.m)
	...
	// how to sort results array (of objects) : by position desc
	// (or seed asc for looks)
	// only for sorting (more advanced `pos` algorithms may be
	// used separately)
	 ...
	 * In particular, rounds like 'Round of 8/16' is called '8ths/16ths Finals'
	 * in other countries, whereas we just hard coded them as string +
	 * Math.pow(2, x).
	 * 
	 * It is therefore possible this should be moved out of tournament and be
	 * mixed in by the user of- rather than the module itself given the
	 * customizations needed. Sensible defaults is the only reason for keeping
	 * it inside tournament.
	 */

	var seNames = [ 'Bronze final', // bronze final match is
	// sometimes parts of the
	// 'finals' round
	'Grand final', // often called just the 'Final'
	'Semi-finals', 'Quarter-finals', 'Round of ' ];

	// when in double elimination we use these 2
	var wbNames = [ 'WB Final', 'WB Semi-finals', 'WB Quarter-finals', 'WB Round of ' ];
	var lbNames = [ 'Grand final', // Strong grand final - but
	// spoiler by prefixing
	// Strong
	...
	// can take a partial id where everything but the match
	// number is left out
	// passed in T is a limited object that gets passed in via
	// the in-mixer
	var roundName = function(T, last, p, partialId) {
		var br = partialId.s, r = partialId.r;

		// sanity
		if (!Number.isFinite(r) || r < 1 || [ T.WB, T.LB ].indexOf(br) < 0) {
			throw new Error('invalid partial id for roundName: ' + partialId);
		}
		var invWB = (br === T.WB && r > p), invSeLB = (last === T.WB && br >= T.LB && r !== 1), invDeLB = (last === T.LB && r > 2 * p);
		...
		if (last === T.WB) {
			if (br === T.LB) {
				return seNames[0]; // already know it is not an
				// invalid match
			}
		...
		if (br === T.LB) {
			if (r >= 2 * p - 3) {
				return lbNames[2 * p - r]; // grand final
				// rounds or LB
				// final
			}
			...
			// Mixed round of %d (always the same as the round
			// before because of feeding)
	...
	var WB = T.WB, LB = T.LB, NA = T.NA, WO = T.WO;

	// map last bracket num to elimination type
	var brackets = [ 'invalid', // put this in here so the Array
	// is aligned right
	'single', 'double', 'triple' // not supported atm
	];
	...
	// shortcut to create a match id as duel tourneys are very
	...
	// get initial players for match i in a power p duel
	// tournament
	...
	var makeFirstRounds = function(size, p, last) {
		var model = Math.pow(2, p); // model >= size (equals iff
		// size is a power of 2)
		var matches = [], lbm1, lbm2, wbm2; // placeholders for
		// LBR1, LBR2 & WBR2
		// (used in even
		// itrs)

		for (var i = 1; i <= model / 2; i += 1) {
			var ps = woMark(seeds(i, p), size), isEven = Number(i % 2 === 0), wbm1 = {
				id : gId(WB, 1, i),
				p : ps
			};
			...
			if (ps[0] === WO || ps[1] === WO) {
				wbm2.p[isEven] = ps[Number(ps[0] === WO)]; // advance
				// winner
				wbm1.m = (ps[0] === WO) ? [ 0, 1 ] : [ 1, 0 ]; // set
				// WO
				// score
				// in
				// wbm1
				...
				if (last >= LB) {
					lbm1.p[isEven] = WO; // wo marker 'lost',
					// so goes to LBR1
					if (lbm1.p[0] === WO && lbm1.p[1] === WO) {
						// NB: here in even itrs when we have
						// rare 2x WO markers in an LBR1 match
						lbm2.p[Number(!isEven)] = WO; // pass
						// it on
						// (w/LBR2's
						// inverse
						// propagation)
						lbm1.m = [ 1, 0 ]; // randomly score
						// one as the winner
					}
				}
			}
			...
			matches.push(wbm1);
			// progressed shells pushed to matches every other
			// iteration
			if (isEven) {
				matches.push(wbm2);
				if (last >= LB) {
	...
	// a match has the form {p: playerArray, s: bracketNum, r:
	// roundNum, m: matchNum}
	// bracket, round and match number are 1 indexed
	var elimination = function(size, p, last, isLong) {
		var invReason = invalid(size, last, {
			limit : 0,
			short : !isLong
		});
		if (invReason !== null) {
			console.log('invalid Duel configuration %dp in %s elimination', size, brackets[last] || 'invalid');
			console.log('reason: ', invReason);
			return [];
		}

		// console.log('creating %dp %s elimination tournament',
		// size, brackets[last]);
		// create round 1,2 in WB & LB
		var matches = makeFirstRounds(size, p, last);

		// remaining WB rounds (which never get WO markers from
		// first fill-in)
		var r, g;
		for (r = 3; r <= p; r += 1) {
		...
		if (last >= LB) {
			for (r = 3; r <= 2 * p - 2; r += 1) {
				// number of matches halves every odd round in
				// losers bracket
		...
		return matches.sort(T.compareMatches); // sort so they
		// can be scored
		// in order
	};

	// progression helpers, winner in `id` goes right to
	// returned id or tournament over
	var progressRight = function(id, underdogWon) {
		var b = id.s, r = id.r, g = id.m, p = this.p, last = this.last, isLong = this.isLong;

		// cases where progression stops for winners
		var isFinalSe = (last === WB && r === p), isFinalDe = (last === LB && b === LB && r === 2 * p), isBronze = (last === WB && b === LB), isShortLbGf = (b === LB && r === 2 * p - 1 && (!isLong || !underdogWon));
		...
		if (b === WB) {
			...
			pos = g % 2; // LBR1 winners move inversely to
			// normal progression
		} else if (interlude.odd(r)) {
			pos = 1; // winner usually takes bottom position
			// in LB
		} else {
			pos = (g + 1) % 2; // normal progression only in
			// even rounds
		}
	...
	var progressDown = function(id, underdogWon) {
		...
		if (b >= last) { // greater than case is for BF in
			// long single elimination
			if (b === WB && isLong && r === p - 1) {
				// if bronze final, move loser to 'LBR1' at
				// mirror pos of WBGF
				return [ gId(LB, 1, 1), (g + 1) % 2 ];
			}
			if (b === LB && r === 2 * p - 1 && isLong && underdogWon) {
				// if double final, then loser moves to the
				// bottom
				return [ gId(LB, 2 * p, 1), 1 ];
			}
			// otherwise always KO if loosing in >= last
			// bracket
			return null;
		}

		// LB drops: on top for (r>2) and (r<=2 if odd g) to
		// match bracket movement
		var pos = (r > 2 || interlude.odd(g)) ? 0 : 1;
		// LBR1 only fed by WBR1 (halves normally), else feed ->
		// r=2x later (w/matching g)
	...
	var unscorable = function(ms, id, score, allowPast) {
		...
		if (m.p[0] === WO || m.p[1] === WO) {
			return 'cannot override score in walkover match';
		}
	...
	// updates matches by updating the given match, and
	// propagate the winners/losers
	var score = function(right, down, ms, id, score) {
		// 0. error handling - if this fails client didnt guard
		// so we log
		var invReason = unscorable(ms, id, score, true);
		if (invReason !== null) {
			console.log('failed scoring Duel match %s with %j', idString(id), score);
			console.log('reason:', invReason);
			return false;
		}
		// console.log('scoring Duel %s with %j', idString(id),
		// score);

		// 1. score given match
		var m = T.findMatch(ms, id);
		m.m = score; // only map scores are relevant for
		// progression

		// helper to insert player adv into [id, pos] from
		// progression functions
		var playerInsert = function(progress, adv) {
			if (progress) {
				var id = progress[0], pos = progress[1], insertM = T.findMatch(ms, id);
				...
				if (insertM.p[(pos + 1) % 2] === WO) {
					insertM.m = (pos) ? [ 0, 1 ] : [ 1, 0 ]; // set
					// WO
					// map
					// scores
					return insertM.id; // this id was won by
					// adv on WO, inform
				}
			}
		};
		...
		var w = (score[0] > score[1]) ? m.p[0] : m.p[1], l = (score[0] > score[1]) ? m.p[1] : m.p[0];

		// an underdog win may force a double match where
		// brackets join
		// currently, this only happens in double elimination in
		// GF1 and isLong
		var underdogWon = (w === m.p[1]);

		// 2. move winner right
		// NB: non-WO match `id` cannot `right` into a WOd match
		// => discard res
		...
		// 4. check if loser must be forwarded from existing WO
		// in LBR1/LBR2
		// NB: underdogWon is never relevant as LBR2 is always
		// before GF1 when p >= 2
		if (dres) {
	...
	var lbPos = function(p, maxr) {
		...
		// works upto and including LB final (gf players must be
		// positioned manually)
		var metric = 2 * p - maxr;
		var k = Math.floor(metric / 2) - 1; // every other
		// doubles
		if (k < 0) {
	...
	var wbPos = function(p, maxr) {
		// similar but simpler, double each round, and note tat
		// ties are + 1
		// works up to and including semis (WBF + BF must be
		// positioned manually)
	...
	var results = function(ms, down) {
		var last = this.last, p = this.p, isLong = this.isLong, res = new Array(this.numPlayers);
		...
		for (var i = 0; i < ms.length; i += 1) {
			...
			var isWbGf = (last === WB && g.id.s === WB && g.id.r === p);
			var isLbGfs = (last === LB && g.id.s === LB && g.id.r >= 2 * p - 1);
			var isLongSemi = (isLong && last === WB && g.id.s === WB && g.id.r === p - 1);
			var canPosition = !isBf && !isWbGf && !isLbGfs && !isLongSemi;
			var maxr = (last === LB && g.id.s === WB) ? down(g.id, false)[0].r : g.id.r;

			// basic positioning handling of players that may
			// merely have reached the match
			for (var j = 0; j < g.p.length; j += 1) {
				var pX = g.p[j] - 1;
				if (pX >= 0) {
					// position as if we got to maxr (i.e. this,
					// or `down` to immediate loss pos)
					// if we cant position using placement, we
					// are in one of the 4 specials
					// we want position 2 in the finals, but 4
					// in BF and longSemis (equivalent)
			...
			if (g.p[0] === WO || g.p[1] === WO || !g.m) {
				...
				// if unscored, can only keep on assuming worst
				// outcome
				continue;
			}

			// when we have scores, we have a winner and a loser
			var p0 = g.p[0] - 1, p1 = g.p[1] - 1, w = (g.m[0] > g.m[1]) ? p0 : p1, l = (g.m[0] > g.m[1]) ? p1 : p0;
			...
			// finals handling (if played) - overwrites earlier
			// handling
			if (isBf) {
				res[l].pos = 4;
				res[w].pos = 3;
	...
	// upcoming match always exists in this as winners/losers
	// are propagated immediately
	var upcoming = function(ms, pId) {
		// a player is only ever in one unscored match for duel
		// tournaments
	...
	function Duel(numPlayers, numBrackets, opts, matches) {
		...
		// isLong is the default final behaviour, which can be
		// turned off via opts.short
		// isLong for WB === hasBF, isLong for LB === hasGf2 (in
		// the sense that they exist)
		this.isLong = true;
		this.limit = 0;
		if (opts) {
	...
	// an instance can be serialized directly by storing
	// inst.matches
	Duel.fromJSON = function(matches, limit) {
		if (!matches.length) {
			console.log('cannot recreate tournament - no matches found');
			return;
		}
		...
		var last = (lbLen > 1) ? LB : WB; // single allows
		// only one match in
		// LB
		...
		var gf2 = T.findMatch(matches, {
			s : LB,
			r : 2 * p,
			m : 1
		});
		var isShort = (last === WB && lbLen === 0) || (last === LB && !gf2);
		return (new Duel(numPlayers, last, {
			short : isShort,
			limit : limit
		}, matches));
	};
	...
	// roundName module
	// Duel.prototype.roundName = require('./duel_names')({LB:
	// LB, WB: WB});
	Duel.prototype.roundName = function(partialId) {
		return roundName({
			LB : LB,
			WB : WB
		}, this.last, this.p, partialId);
	};

	Duel.prototype.isDone = function() {
		if (!this.isLong) {
			// in short mode, a couple of simplifications apply:
			// the last match is always the final - and the
			// final is always scorable last
			var gf = this.matches[this.matches.length - 1];
			return Array.isArray(gf.m);
		}

		// otherwise we have either a bronze final or a
		// POTENTIAL double grand final
		if (this.matches.every(interlude.get('m'))) {
			return true; // covers single elimination cases
			// bronze final and long double
		}

		// but allDone misses a potential early exit from a
		// short GF in double elimination
